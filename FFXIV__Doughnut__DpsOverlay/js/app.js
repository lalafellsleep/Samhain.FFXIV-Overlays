...
function encounterAct(e, n)
{
	...
	for(var user in e.detail.Combatant)
	{
		...
		combatant.push(
		{
			...
			"combatant":c,
			"avatar":false,
			"color":"#"+(Math.random()*99)+(Math.random()*99)+(Math.random()*99)
		});
	}
	...
	for(var user in combatant)
	{
		...
		combatant[user].hvalue = parseInt(combatant[user].combat["damage%"]);
		combatant[user].color = colors[idx];
		idx++;
	}
	
	$(".data").html("DUR : "+e.detail.Encounter.duration+"<br> RDPS : "+e.detail.Encounter.encdps+"<br>Tot. : "+e.detail.Encounter.damage);
	$(".chart").donutpie("update", combatant);
	$(".title").html(e.detail.Encounter.title);
	$(".users").html("");
	for(var user in combatant)
	{
		if(parseInt(combatant[user].combat["damage%"]) == 0) continue;
		$(".users").append("<div style=\"height:14px; line-height:14px; font-size:12px; font-family:\'돋움\'; margin-bottom:3px; color:#FFF;\"><div style='width:10px; height:14px; margin-right:3px; background:"+colors[combatant[user].rank]+"; display:inline-block;'></div>["+combatant[user].combatant.Job.toUpperCase()+"]"+combatant[user].name+" ("+combatant[user].combat["encdps"]+"DPS)</div>");
	}
	$(".users").append("<div style=\"height:14px; line-height:14px; font-size:12px; font-family:\'돋움\'; margin-bottom:3px; color:#FFF;\">딜지분 1% 미만은 표기되지 않음.</div>");
}
