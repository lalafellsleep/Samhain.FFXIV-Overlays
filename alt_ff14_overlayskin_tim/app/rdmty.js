...
var ____Class3=React.Component;for(var ____Class3____Key in ____Class3){if(____Class3.hasOwnProperty(____Class3____Key)){Combatants[____Class3____Key]=____Class3[____Class3____Key];}}var ____SuperProtoOf____Class3=____Class3===null?null:____Class3.prototype;Combatants.prototype=Object.create(____SuperProtoOf____Class3);Combatants.prototype.constructor=Combatants;Combatants.__superConstructor__=____Class3;function Combatants(){"use strict";if(____Class3!==null){____Class3.apply(this,arguments);}}
    ...
    Object.defineProperty(Combatants.prototype,"render",{writable:true,configurable:true,value:function() {"use strict";
        ...
        for (var i = 0; i < names.length; i++) {
            ...
            if(isSelf)
            {
                combatant.name = 'Pizza Cat'
                /* if($("input:checkbox[id='myname']").is(":checked"))
                {
                    combatant.name = 'Pizza Cat'//$("#mynameval").val();
                } */
            }
...
/*                          perSecond: combatant.ParryPct,
                    ...
                    var jobAbbr = combatant.JobN ? '[' + combatant.JobN + ']' : "";
                    var strlen = combatant.maxhit.toString().length;
                    var nameLen = virtualname.toString().length + 6
                    var spaces = 40 - strlen;
                    var padLeft = spaces/2 + strlen;
                    var spacesName = 30 - nameLen;
                    var padLeftName = spacesName/2 + nameLen;
                    var maxhit = combatant.maxhit.toString()
                    var maxHitName = maxhit.substring(0, maxhit.lastIndexOf('-'))
                    var maxHitValue = maxhit.substring(maxhit.lastIndexOf('-') + 1)
                    var maxHitSplit = combatant.maxhit.toString().split("-");
                    /* if (maxHitSplit == null || maxHitSplit[1] == null || maxHitSplit[0] == null) {
                        maxHitSplit[0] = 'N/A';
                        maxHitSplit[1] = '0';
                    } */
                    if (maxhit == null) {
                    ...
                    stats = {
                            ...
                            /*displayJobName: combatant.JobN,*/
                            job: combatant.Job || '',
                            characterName: "\xa0" + virtualname.toString().padRight("\xa0", 26),
                            ...
                            totalFormatted: React.createElement("span", {className:"datas"},
                                ...
                                /*RAAA*/
                                /*virtualname.toString().padLeft("\xa0", 20) + */
                                /*'[' + combatant.JobN + ']' */
                                /*combatant.maxhit.toString().padLeft("\xa0", padLeft).padRight("\xa0", 40) */
                                //jobAbbr
                                //+ 
                                ((Math.round(combatant.encdps) + ' DPS').toString().padLeft("\xa0", 10) 
                                + ('(' + combatant['crithit%']).toString().padLeft("\xa0", 8) + ')' 
                                /*+ combatant.maxhit.toString().padLeft("\xa0", padLeft).padRight("\xa0", 40)*/
                                + maxHitValue.padLeft("\xa0", 10) + ' - '
                                + maxHitName.padRight("\xa0", 27)
                                + ('[' + combatant.deaths).toString().padLeft("\xa0", 4) + ']' ),
                                /*+ ('[' + combatant.misses).toString().padLeft("\xa0", 4) + ']'),*/
